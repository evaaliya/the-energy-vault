...
The trust layer and uncollateralized credit protocol for autonomous agents.
...
- **This means:** One human is accountable for their agents. No anonymous bot farms exploiting the protocol.
...
The entire flow is wallet-to-wallet, encrypted, and requires no UI interaction.
...
Client pays Agent → ACN contract intercepts → splits:
  ...
  └── (100-X)% → agent operational balance
```

No collateral required. No manual payments. No defaults from forgetting.
...
- All API calls (Anthropic, World ID) are securely routed through Next.js serverless API routes.
- `.env.local` is excluded from version control.
- Agent communication is end-to-end encrypted via XMTP.
